...
Codex review is the default when no engine is set. It uses `gpt-5.6-sol` with `max` reasoning by default, then retries once with `gpt-5.6-terra` only when the account cannot access Sol. Claude review is optional and uses `claude-fable-5` by default.
...
- Reviewer subprocesses preserve engine authentication and non-credentialed proxy variables needed by headless or restricted-network environments. They strip process-injection and Git override variables, and fail closed when a configured proxy URL is credentialed or malformed.
...
[OpenAI's model guidance](https://developers.openai.com/api/docs/guides/latest-model) identifies Sol as the GPT-5.6 frontier-capability route and documents `max` support. Autoreview uses `max` by default for quality-first reviews; use a lower effort when latency or cost matters.
...
| **claude**   | `--safe-mode --setting-sources user --strict-mcp-config --disallowedTools mcp__*`; auto-memory and filesystem/shell tools disabled; empty external workspace; WebSearch by default (Claude Code `v2.1.169+`; default `claude-fable-5` requires `v2.1.170+`) | Claude Code [CLI reference](https://code.claude.com/docs/en/cli-reference) |
...
- runs Claude with `--safe-mode` (`v2.1.169+` for non-Fable models; `v2.1.170+` for the default `claude-fable-5`), `--setting-sources user`, MCP and auto-memory disabled, no filesystem/shell tools, an empty external workspace, and `--fallback-model` when set
