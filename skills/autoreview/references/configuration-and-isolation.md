# Configuration and isolation reference

Use this reference when configuring non-default engines, installing on alternate agent paths, diagnosing isolation failures, or checking the helper's complete capability surface.

## Contents

- [Skill path](#skill-path-set-once)
- [Oversized bundles](#oversized-bundles)
- [Parallel closeout](#parallel-closeout)
- [Models and thinking](#models-and-thinking)
- [Review engine isolation](#review-engine-isolation)
- [Detailed helper capabilities](#detailed-helper-capabilities)
...
```bash
# Source checkout of this skills collection:
...
When using Codex, set `AGENTS_HOME="${CODEX_HOME:-$HOME/.codex}"` for a Codex-global install. When using Claude Code, set `AGENTS_HOME="$HOME/.claude"`.
...
| ------------------- | -------------------------------------------------- | ----------------------------------------------------- |
| **codex** (default) | `gpt-5.6-sol` -> `gpt-5.6-terra` on access failure | Quality-first closeout default                        |
...
[OpenAI's model guidance](https://developers.openai.com/api/docs/guides/latest-model) identifies Sol as the GPT-5.6 frontier-capability route and documents `max` support. This merged skill intentionally uses `max` as the quality-first default. Users can lower reasoning through `--thinking` or the documented environment variables when latency or cost matters more.
